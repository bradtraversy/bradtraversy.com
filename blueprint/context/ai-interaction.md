...
- Build one small, reviewable slice at a time and stop for Brad's approval
- Do not implement multiple pages or build-plan items in one pass
- Do not start the dev server unless Brad explicitly asks
...
3. **Implement** - Build one small step from the spec at a time, not the whole
   feature at once. Stop after every step so Brad can review it before the next
   step begins.
...
9. **Checkpoint (optional)** - after an approved milestone `/implement` offers a quick
